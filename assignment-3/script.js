...
function getRatioText(tutors, students) {
  if (tutors === 0) {
    return `no tutors`;
  }
  ...
  return `${rounded} student${rounded === 1 ? "" : "s"} per tutor`;
}
...
function renderCounts() {
  ...
  cards.forEach(card => {
    ...
    const ratioText = getRatioText(tutors, students);
    statusText.textContent = `${status.icon} ${status.label} (${ratioText})`;
